...
const IncomeForm = (props) => {
  ...
  const submitHandler = (e) => {
    ...
    const incomeData=  {income: referentIncome.current.value, date: referentDate};
  ...
  <main className="flex items-center justify-center h-screen bg-center bg-cover" style={{ backgroundImage: `url(${image1})` }}>
    <div className="w-full md:w-7/12 h-1/2 bg-red-500 p-10 absolute p-4 rounded-lg shadow-lg flex flex-col">
 ...
 <div className="py-2 flex items-center">
  <Link to="income" className={`bg-blue-500 mt-10 text-white text-center py-2 px-4 rounded-lg m-auto ${isLinkDisabled ? 'pointer-events-none' : "" } sm:w-24 md:w-32 lg:w-48 xl:w-64`}>Calculate</Link>
 </div>
  </form>
            </div>
         </main>
