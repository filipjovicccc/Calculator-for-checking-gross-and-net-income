...
const IncomeForm = ({formIsVisible, onAdd, handleChange, setSelectedGroup, selectedGroup, setFormIsVisible}) => {
  ...
  //   const referentGroup = selectedGroup;
  //   if(referentIncome.trim().length === 0){
  //     return alert("Please enter valid information")
  //   }
  //   const incomeData = { income: referentIncome, group: referentGroup };
  //   if (!incomeData.group) {
  ...
  //     return;
  // }
  //   onAdd(console.log(incomeData));
  //   incomeRef.current.value = "";
  //   setSelectedGroup("Weekly");
    ...
    <button className="bg-green-500 text-white  flex self-start  py-2 px-4 rounded-lg mr-2 sm:mr-0 md:mr-2 lg:mr-4 xl:mr-6"  onClick={() => setFormIsVisible(!formIsVisible)}>
      ...
      <form onSubmit={submitHandler} className="mt-4 divide-y divide-gray-400">
        <div className="py-2">
          ...
          <select onChange={handleChange} value={selectedGroup}  className="border border-gray-400 rounded-lg py-2 px-3">
        ...
        <div className="py-2">
          ...
          <button type="submit" className="bg-green-500 text-white  py-2 px-4 rounded-lg mr-2 sm:mr-0 md:mr-2 lg:mr-4 xl:mr-6">Gross Income</button>
    <button className="bg-green-500 text-white py-2 px-4 rounded-lg sm:ml-0 md:ml-2 lg:ml-4 xl:ml-6">Net Income</button>
  </div>
  </div>
 <div className="py-2 flex items-center">

  <button className="bg-darkGray text-white  w-96 flex  items-center justify-center my-auto mt-10 mx-32 text-center py-2 px-5 rounded-lg mr-3 sm:mr-0 md:mr-2 lg:mr-4 xl:mr-6">Calculate</button>
 </div>
  </form>
            </div>
         </main>
